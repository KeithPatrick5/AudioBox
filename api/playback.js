...
async function fetchJson(url, timeoutMs = 4200) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { Accept: 'application/json', 'User-Agent': 'AudioBox/2.1 playback-hunter' }
    });
...
function titleScore(targetTitle, candidateTitle) {
  ...
  const got = normalize(candidateTitle);
...
function metadataToPlayback(payload, identifier, provider, score, expectedMinutes) {
  ...
  return {
    ...
    matchedTitle: clean(md.title, 300),
...
async function hunt(book) {
  ...
  const creator = safeLucene(book.author);
  if (!title) return null;

  const searches = [
    {
      provider: 'Internet Archive / LibriVox',
      q: `collection:librivoxaudio AND mediatype:audio AND title:("${title}")${creator ? ` AND creator:("${creator}")` : ''}`
    },
    {
      provider: 'Internet Archive / Audiobooks',
      q: `collection:audio_bookspoetry AND mediatype:audio AND title:("${title}")${creator ? ` AND creator:("${creator}")` : ''}`
    },
    {
      provider: 'Internet Archive / Open Audio',
      q: `mediatype:audio AND title:("${title}")${creator ? ` AND creator:("${creator}")` : ''} AND (subject:(audiobook) OR subject:("audio book") OR description:(audiobook) OR description:(librivox))`
    }
  ];
  ...
  const candidates = settled.flatMap(x => x.status === 'fulfilled' ? x.value : [])
    .filter(x => x.doc?.identifier)
    .sort((a, b) => b.score - a.score)
    .slice(0, 4);

  if (!candidates.length) return null;
  const verified = (await Promise.all(candidates.map(item => verifyCandidate(book, item))))
    .filter(Boolean)
